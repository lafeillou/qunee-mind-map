...
window.CodeMirror = (function() {
    ...
    if (opera_version && opera_version >= 15) { opera = false;
        webkit = true; }
    ...
    function CodeMirror(place, options) {
        ...
        var hasFocus;
        try { hasFocus = (document.activeElement == display.input); } catch (e) {}
    ...
    function makeDisplay(place, docStart) {
        ...
        if (ie_lt8) { d.gutters.style.zIndex = -1;
        ...
        if (khtml) { d.inputDiv.style.height = "1px";
    ...
    function visibleLines(display, doc, viewPort) {
        ...
        else if (viewPort) { top = viewPort.top;
    ...
    function coordsCharInner(cm, lineObj, lineNo, x, y) {
        ...
        for (;;) {
            ...
            if (middleX > x) { to = middle;
                toX = middleX; if (toOutside = wrongLine) toX += 1000;
                dist = step; } else { from = middle;
                fromX = middleX;
                fromOutside = wrongLine;
                dist -= step; }
        }
    ...
    function operation(cm1, f) {
        return function() {
            ...
            try { var result = f.apply(cm, arguments); } finally { if (withOp) endOperation(cm); }
            return result;
        };
    }
    ...
    function docOperation(f) {
        return function() {
            ...
            try { result = f.apply(this, arguments); } finally { if (withOp) endOperation(this.cm); }
            return result;
        };
    }
    ...
    function runInOp(cm, f) {
        var withOp = !cm.curOp,
            result;
        if (withOp) startOperation(cm);
        try { result = f(); } finally { if (withOp) endOperation(cm); }
        return result;
    }
    ...
    function fastPoll(cm) {
        ...
        function p() {
            ...
            if (!changed && !missed) { missed = true;
                cm.display.poll.set(60, p); } else { cm.display.pollingFast = false;
                slowPoll(cm); }
        }
    ...
    function posFromMouse(cm, e, liberal) {
        ...
        try { x = e.clientX;
            y = e.clientY; } catch (e) { return null; }
    ...
    function handleKeyBinding(cm, e) {
        ...
        if (handled) {
            e_preventDefault(e);
            restartBlink(cm);
            if (ie_lt9) { e.oldKeyCode = e.keyCode;
                e.keyCode = 0; }
    ...
    function onKeyPress(e) {
        ...
        var keyCode = e.keyCode,
            charCode = e.charCode;
        if (opera && keyCode == lastStoppedKey) { lastStoppedKey = null;
    ...
    function widgetOperation(f) {
        return function() {
            ...
            if (withOp) startOperation(this.cm);
            try { var result = f.apply(this, arguments); } finally { if (withOp) endOperation(this.cm); }
            return result;
        };
    }
    ...
    function rebaseHistArray(array, from, to, diff) {
        for (var i = 0; i < array.length; ++i) {
            ...
            for (var j = 0; j < sub.changes.length; ++j) {
                ...
                if (!sub.copied) { cur.from = copyPos(cur.from);
